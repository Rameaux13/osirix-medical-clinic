...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen bg-theme-primary theme-transition">
      ...
      <header className="fixed top-0 w-full bg-theme-card shadow-theme-lg z-50 theme-transition">
        <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-12">
          <div className="flex items-center justify-between h-20 sm:h-24">
            ...
            {/* Boutons Auth + Dark Mode (Desktop) */}
          ...
          {mobileMenuOpen && (
            <div className="md:hidden absolute top-20 sm:top-24 left-0 right-0 bg-theme-card shadow-theme-xl rounded-b-2xl border-t border-theme theme-transition">
              <div className="px-6 py-6 space-y-4">
                <a href="#accueil" onClick={(e) => { e.preventDefault(); setMobileMenuOpen(false); document.getElementById('accueil')?.scrollIntoView({ behavior: 'smooth' }); }} className="block nav-link-theme text-xl py-3 cursor-pointer">Accueil</a>
                <a href="#services" onClick={(e) => { e.preventDefault(); setMobileMenuOpen(false); document.getElementById('services')?.scrollIntoView({ behavior: 'smooth' }); }} className="block nav-link-theme text-xl py-3 cursor-pointer">Services</a>
                <a href="#avis" onClick={(e) => { e.preventDefault(); setMobileMenuOpen(false); document.getElementById('avis')?.scrollIntoView({ behavior: 'smooth' }); }} className="block nav-link-theme text-xl py-3 cursor-pointer">Avis</a>
                <a href="#rendez-vous" onClick={(e) => { e.preventDefault(); setMobileMenuOpen(false); document.getElementById('accueil')?.scrollIntoView({ behavior: 'smooth' }); }} className="block nav-link-theme text-xl py-3 cursor-pointer">Rendez-vous</a>
                <a href="#contact" onClick={(e) => { e.preventDefault(); setMobileMenuOpen(false); document.getElementById('contact')?.scrollIntoView({ behavior: 'smooth' }); }} className="block nav-link-theme text-xl py-3 cursor-pointer">Contact</a>
                <div className="pt-4 space-y-3 border-t border-theme">
                  <Link href="/login" onClick={() => setMobileMenuOpen(false)} className="w-full btn-outline text-xl py-4 block text-center">Connexion</Link>
                  <Link href="/register" onClick={() => setMobileMenuOpen(false)} className="w-full btn-primary text-xl py-4 block text-center">Inscription</Link>
                </div>
              </div>
            </div>
          )}
      ...
      {/* ============================== HERO SECTION ============================== */}
      <section id="accueil" className="min-h-screen bg-gradient-to-br from-primary-600/90 to-primary-700/90 bg-cover bg-center flex items-center justify-center text-white relative overflow-hidden pt-20 sm:pt-24" style={{ backgroundImage: `linear-gradient(rgba(0, 109, 101, 0.75), rgba(0, 109, 101, 0.75)), url('https://images.unsplash.com/photo-1532938911079-1b06ac7ceec7?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1480&q=80')`, backgroundSize: 'cover', backgroundPosition: 'center center' }}>
      ...
      {/* 
        CORRECTION : 
        - Mode clair : fond blanc pur, texte noir, pas de gris pâle
        - Mode sombre : fond gris foncé, texte clair
        - Suppression des décorations grises pâles
      */}
              ...
              <div className="flex flex-wrap gap-4 sm:gap-6 mb-6 sm:mb-8">
                <Link href="#" className="group" aria-label="Facebook">
                  <div className="w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 bg-secondary-500 dark:bg-secondary-600 rounded-full flex items-center justify-center hover:bg-secondary-600 dark:hover:bg-secondary-500 transition-all duration-300 hover:scale-110 shadow-xl hover:shadow-2xl theme-transition">
                    <Image src="/facebook-logo.png" alt="Facebook" width={48} height={48} className="w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12" />
                  </div>
                </Link>
                <Link href="#" className="group" aria-label="Instagram">
                  <div className="w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 bg-secondary-500 dark:bg-secondary-600 rounded-full flex items-center justify-center hover:bg-secondary-600 dark:hover:bg-secondary-500 transition-all duration-300 hover:scale-110 shadow-xl hover:shadow-2xl theme-transition">
                    <Image src="/logo-Instagram.png" alt="Instagram" width={48} height={48} className="w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12" />
                  </div>
                </Link>
                <Link href="#" className="group" aria-label="WhatsApp">
                  <div className="w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 bg-secondary-500 dark:bg-secondary-600 rounded-full flex items-center justify-center hover:bg-secondary-600 dark:hover:bg-secondary-500 transition-all duration-300 hover:scale-110 shadow-xl hover:shadow-2xl theme-transition">
                    <Image src="/whatsapp.png" alt="WhatsApp" width={100} height={100} className="w-8 h-8 sm:w-10 sm:h-10 md:w-15 md:h-17" />
                  </div>
                </Link>
              </div>
