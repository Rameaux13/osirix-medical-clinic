...
export default function ResetPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://osirix-backend.onrender.com'}/auth/reset-password`, {
        method: 'POST',
        headers: {
      ...
      setMessage({
        type: 'success',
        text: data.message || 'Mot de passe réinitialisé avec succès !',
      });
      ...
      setMessage({
        type: 'error',
        text: error.message || 'Token invalide ou expiré. Veuillez refaire une demande.',
      });
    } finally {
      setIsLoading(false);
    }
      ...
      <div className="max-w-md w-full">
        ...
        <div className="bg-white rounded-xl md:rounded-2xl shadow-xl p-6 md:p-8 border-2 border-primary-100">
          ...
          <form onSubmit={handleSubmit} className="space-y-5 md:space-y-6">
            ...
            {/* Nouveau mot de passe */}
            <div>
              <label htmlFor="newPassword" className="block text-xs md:text-sm font-semibold text-primary-800 mb-2">
                Nouveau mot de passe
              </label>
              <div className="relative">
                <input
                  ...
                  className="w-full px-3 py-2.5 md:px-4 md:py-3 pr-10 md:pr-12 border-2 border-primary-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-300 focus:border-primary-500 transition-all text-sm md:text-base"
                  placeholder="Min. 6 caractères"
                />
                <button
                  type="button"
                  onClick={() => setShowPassword(!showPassword)}
                  className="absolute inset-y-0 right-0 pr-2 md:pr-3 flex items-center text-primary-500 hover:text-primary-700"
                >
                  {showPassword ? (
            ...
            <div>
              <label htmlFor="confirmPassword" className="block text-xs md:text-sm font-semibold text-primary-800 mb-2">
              ...
              <input
                ...
                className="w-full px-3 py-2.5 md:px-4 md:py-3 border-2 border-primary-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-300 focus:border-primary-500 transition-all text-sm md:text-base"
            ...
            <button
              ...
              className="w-full bg-gradient-to-r from-secondary-500 to-secondary-600 hover:from-secondary-600 hover:to-secondary-700 disabled:from-neutral-400 disabled:to-neutral-500 text-white font-bold py-3 md:py-3.5 px-6 rounded-xl shadow-lg hover:shadow-xl disabled:cursor-not-allowed transition-all text-sm md:text-base"
            >
              {isLoading ? (
